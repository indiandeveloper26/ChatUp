...
export default function ChatRoom() {
    ...
    // FIX: Dependency se 'typingUser' hata diya taaki sirf message aane par scroll ho
    useEffect(() => {
        messagesEndRef.current?.scrollIntoView({ behavior: "smooth" });
    }, [filtered.length]); // filtered.length use karna zyada stable hai
        ...
        <div className="flex flex-col h-screen bg-[#0B0E11] text-slate-100 overflow-hidden font-sans">
            ...
            <header className="flex items-center justify-between px-3 py-3 bg-[#121B22]/95 backdrop-blur-lg border-b border-white/5 sticky top-0 z-50">
                ...
                <div className="flex items-center gap-1">
                    <button onClick={handleVideoCall} className="p-2.5 text-slate-300 hover:text-emerald-400 hover:bg-white/5 rounded-full transition-all">
                        <Video size={22} />
                    </button>
                    <button onClick={handleAudioCall} className="p-2.5 text-slate-300 hover:text-emerald-400 hover:bg-white/5 rounded-full transition-all">
                        <Phone size={20} />
                    </button>
                    <button onClick={handleOTP} className="p-2.5 text-slate-400 hover:bg-white/5 rounded-full transition-all">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar bg-[url('https://w0.peakpx.com/wallpaper/508/446/HD-wallpaper-whatsapp-dark-mode-doodle-pattern-whatsapp-messenger.jpg')] bg-repeat">
                ...
                {/* Typing Indicator (Ab ye auto-scroll trigger nahi karega) */}
                <AnimatePresence>
                    {typingUser === id && (
